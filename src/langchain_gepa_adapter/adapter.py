...
logger = logging.getLogger("langchain_gepa_adapter.adapter")
...
def _default_reflective_record(
    data: DataInst, state: RolloutState, score: float, feedback: str
) -> Mapping[str, Any]:
...
class LangChainGEPAAdapter(GEPAAdapter):
    ...
    def evaluate(
        ...
        def run_one(idx: int, example: DataInst):
            ...
            if error is not None:
                ...
                logger.debug(
                    "example %d ok in %.0fms (score=%.2f)", idx, elapsed_ms, score
                )
    ...
    def make_reflective_dataset(
        ...
        records = [
            self.reflective_record_fn(
                traj["data"], traj["state"], traj["score"], traj["feedback"]
            )
